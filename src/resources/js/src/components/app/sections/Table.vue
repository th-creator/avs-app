...
<script setup>
    ...
    const rows = computed(() => {
        return sectionsStore.sections.length > 0 ? sectionsStore.sections : [];
        });
    ...
    onMounted(async () => {
        sectionsStore.sections.length == 0 && (isloading.value = true)
    ...
    const deleteData = (data) => {
        ...
        .then((result) => {
            if (result.value) {
                sectionsStore.destroy(data.id).then(res => {
