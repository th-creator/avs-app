...
<script setup>
    ...
    const rows = computed(async() => {
        let data = await paymentsStore.checkPayments.length > 0 ? paymentsStore.checkPayments : []
    ...
    const searchPayments = async () => {
        ...
        await paymentsStore.fetchchecks(search.value.from,search.value.to)
        isloading.value =false
        
        choosenData.value = paymentsStore.checkPayments.reduce((accumulator, current) => {
            let duplicate = accumulator.find(item => item.bank_receipt === current.bank_receipt);
            if (duplicate) {
        ...
        total.value = choosenData.value.reduce((total, payment) => {
            ...
            return total + Number(payment.total)
        }, 0)
    ...
    const exportToExcel = () => {
        ...
        const attendanceData = choosenData.value.map(res => ({'Chèque': res.bank_receipt, 'Bank': res.bank, 'Montant': res.total, 'Date': res.date}))
